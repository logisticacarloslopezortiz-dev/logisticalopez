// supabase/functions/getVapidKey/index.ts
...
Deno.serve(async (req: Request) => {
  ...
  try {
    const publicKey = Deno.env.get('PUBLIC_VAPID_KEY') ?? Deno.env.get('VAPID_PUBLIC_KEY')

    if (!publicKey) {
      return jsonResponse(
        { error: 'missing_public_vapid_key' },
        500,
        req
      )
    }
    ...
    if (!isValidVapid(publicKey)) {
      ...
      return jsonResponse(
        { error: 'invalid_public_vapid_key' },
        500,
        req
      )
    }
