...
-- 4.1 Dispatch Notification -> REMOVED (Legacy)
...
begin
  ...
  set
    ...
    tracking_data = case when p_tracking_entry is not null then jsonb_build_array(p_tracking_entry) else o.tracking_data end
...
-- [ARQUITECTURA SANA] Trigger simplificado: Solo genera eventos
...
declare
  ...
  v_event_type text;
begin
  -- Caso 1: Orden Creada (INSERT)
  if (TG_OP = 'INSERT') then
    v_event_type := 'created';
    v_payload := jsonb_build_object('order_id', new.id, 'status', new.status);
    
    -- Event Sourcing
    insert into public.order_events(order_id, event_type, payload)
    values (new.id, v_event_type, v_payload);
    
    -- Despachar al Outbox
    perform public.dispatch_notification(v_event_type, v_payload, 'order_created_' || new.id);
    return new;
  end if;
  ...
  if (TG_OP = 'UPDATE' and new.status is distinct from old.status) then
    ...
    v_payload := jsonb_build_object('order_id', new.id, 'status', new.status, 'old_status', old.status);

    -- Event Sourcing
    insert into public.order_events(order_id, event_type, payload)
    values (new.id, v_event_type, v_payload);

    -- Despachar al Outbox (Solo estados relevantes definidos en templates)
    perform public.dispatch_notification(v_event_type, v_payload, 'order_status_' || new.id || '_' || new.status);
  end if;
  return new;
end;
$$;
