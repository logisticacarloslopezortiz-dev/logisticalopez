...
-- 2.3.a NOTIFICATION LOGS (para auditoría de envíos)
...
BEGIN
    ...
    -- NOTIFICATION LOGS
    DROP POLICY IF EXISTS "Usuarios pueden ver sus propios logs de notificación" ON public.notification_logs;
...
-- Notification Logs
CREATE POLICY "Usuarios pueden ver sus propios logs de notificación"
...
create table if not exists public.notification_templates (
  ...
  role text not null check (role in ('cliente','colaborador','admin')),
  status text,
...
create index if not exists idx_notification_templates_event_role
  on public.notification_templates(event_type, role, coalesce(status, ''));
...
values
  ('created','cliente',null,'es','Orden creada','Tu orden #{{id}} fue creada correctamente. Te avisaremos cada avance.',true),
  ('created','admin',null,'es','Nueva orden creada','Se creó la orden #{{id}}. Requiere asignación.',true),
  ('created','colaborador',null,'es','Nueva orden disponible','Hay una nueva orden #{{id}} pendiente de asignación.',true)
...
values
  -- Cliente
  ('status_changed','cliente','aceptada','es','Actualización de tu orden','Tu orden #{{id}} ha sido aceptada',true),
  ('status_changed','cliente','en_camino_recoger','es','Actualización de tu orden','Tu orden #{{id}}: el repartidor va en camino a recoger',true),
  ('status_changed','cliente','cargando','es','Actualización de tu orden','Tu orden #{{id}}: tu carga está en proceso',true),
  ('status_changed','cliente','en_camino_entregar','es','Actualización de tu orden','Tu orden #{{id}} va en camino a entregar',true),
  ('status_changed','cliente','entregada','es','Actualización de tu orden','Tu orden #{{id}} ha sido entregada',true),
  ('status_changed','cliente','cancelada','es','Actualización de tu orden','Tu orden #{{id}} ha sido cancelada',true),
  -- Colaborador
  ('status_changed','colaborador','aceptada','es','Actualización de tu trabajo','Orden #{{id}} aceptada',true),
  ('status_changed','colaborador','en_camino_recoger','es','Actualización de tu trabajo','Orden #{{id}}: en camino a recoger',true),
  ('status_changed','colaborador','cargando','es','Actualización de tu trabajo','Orden #{{id}}: cargando mercancía',true),
  ('status_changed','colaborador','en_camino_entregar','es','Actualización de tu trabajo','Orden #{{id}}: en ruta a entregar',true),
  ('status_changed','colaborador','entregada','es','Actualización de tu trabajo','Orden #{{id}}: entrega completada',true),
  ('status_changed','colaborador','cancelada','es','Actualización de tu trabajo','Orden #{{id}} cancelada',true)
on conflict do nothing;
